...
# Create an 'outputs' directory if it doesn't already exist
...
write.csv(
  comparison_table,
  file = file.path("outputs", "Weibull_vs_Gamma_Ireland.csv"),
  row.names = FALSE
)

cat("\nComparison table saved to: outputs/Weibull_vs_Gamma_Ireland.csv\n")
...
ggsave(
  paste("Tornado_Diagram_", country_name[1], ".png", sep = ""),
  plot = tornado_plot,
  width = 8,
  height = 4,
...
ggsave(
  paste("Unlimited_Tornado_Diagram_", country_name[1], ".png", sep = ""),
...
# - /outputs or /results: Generated outputs (plots, tables, CSVs)
# - /docs: Documentation and manuscripts
#
# By saving outputs to an "outputs/" folder, we:
...
# 1. Check if the outputs/ folder exists
# 2. Create it if it doesn't exist
# 3. Construct the full file path for our plot
# 4. Save the plot with ggsave at high resolution

# Check if the "outputs" folder exists, and create it if not
# dir.exists() returns TRUE if the folder exists, FALSE otherwise
# The ! operator means NOT, so !dir.exists() means "if NOT exists"
# This is a safe approach - it won't error if the folder already exists
# Health economic value: Ensuring reproducibility across different environments
if (!dir.exists("outputs")) {

  # Create the outputs folder
  # dir.create() creates a new folder
  # recursive = TRUE means it will create parent folders if needed
  # For example, if we wanted "outputs/plots/tornado", it would create all levels
  # Health economic interpretation: Setting up organized output structure
  # Modeling interpretation: Ensuring the target folder exists before saving
  dir.create("outputs", recursive = TRUE)

  # Print a message confirming folder creation
  # This appears in the console for transparency
  # Health economic value: Transparent reporting of file operations
  print("Created 'outputs' folder for results")
}
...
ggsave(
  ...
  # Example: "outputs/tornado_top10_ireland.png"
  filename_top10,
...
# - Created outputs/ folder if needed
# - Saved high-resolution PNG of top 10 tornado plot
# - Filename: outputs/tornado_top10_<country>.png (e.g., outputs/tornado_top10_ireland.png)
...
# This creates filenames like: outputs/tornado_top10_unlimited_ireland.png
# Health economic value: Clear, organized file naming
file_unlimited_top10 <- file.path(
  # Save in the same outputs folder
  "outputs",
...
# -----------------------------------------------------------------------------
# At this point, you should have FOUR tornado plot files in the outputs/ folder:
#
...
#
# □ Six files exist in outputs/ (2 per country: limited and unlimited)
...
# FILE ORGANIZATION:
# -----------------------------------------------------------------------------
# We use an "outputs/" subdirectory to:
# - Keep analysis outputs separate from code and input data
# - Make it easy to find all generated files
# - Allow adding outputs/ to .gitignore if desired (to avoid tracking large files)
#
# FILE NAMING CONVENTION:
# -----------------------------------------------------------------------------
...
# Create outputs directory if it doesn't exist
# ----------------------------------------------
# dir.exists(): Checks if the "outputs" directory exists
# dir.create(): Creates the directory if it doesn't exist
#   - recursive = TRUE: Creates parent directories if needed
#   - This is safe to run even if the directory already exists
#
# This ensures the directory exists before we try to save files to it,
# preventing "path not found" errors
#
# Health economic value: Organized file structure facilitates reproducibility
if (!dir.exists("outputs")) {
  dir.create("outputs", recursive = TRUE)
...
#   - On Windows: "outputs\\PSA_Summary_NMB_Ireland.csv"
#   - On Mac/Linux: "outputs/PSA_Summary_NMB_Ireland.csv"
#
...
output_filename <- file.path("outputs", paste0("PSA_Summary_NMB_", country_name, ".csv"))
...
formatted_filename <- file.path("outputs", paste0("PSA_Summary_NMB_Formatted_", country_name, ".csv"))
