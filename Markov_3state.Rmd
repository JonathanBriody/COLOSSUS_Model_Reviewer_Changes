...
#===============================================================================
# DEFINE UTILITY SCENARIO FLAG
#===============================================================================

# This flag controls which utility values are used in the model:
#   - "ramsey_basecase"           → Use Ramsey et al. (2000) values (0.85, 0.65) unchanged
#   - "population_norm_adjusted"  → Use mCRC multipliers × country/age population norms
#
# DEFAULT: Use Ramsey base case (preserves all published results)
utility_scenario <- "ramsey_basecase"

# TO RUN POPULATION NORM SCENARIO: Uncomment the line below
# utility_scenario <- "population_norm_adjusted"

# NOTE: The scenario analysis chunk (added later in this document) will run
#       BOTH scenarios automatically for comparison and write results to CSV.
#       This flag only controls which scenario is used for the main model run.


#===============================================================================
# APPLY UTILITY SCENARIO SELECTION
#===============================================================================

# IMPLEMENTATION LOGIC:
# ---------------------
# 1. Call get_utility_scenario() with current country_name and patient age
# 2. Extract u_PFS and u_PD from the returned list
# 3. Override u_F and u_P (defined earlier as Ramsey values)
# 4. Print diagnostic information for transparency
#
# RESULT:
# -------
# - If utility_scenario = "ramsey_basecase":
#     u_F and u_P remain unchanged (0.85, 0.65)
# - If utility_scenario = "population_norm_adjusted":
#     u_F and u_P are replaced with country/age-adjusted values

# Define patient age for population norm lookup
# ----------------------------------------------
patient_age <- 65
# Typical age for mCRC diagnosis is 60-70 years
# We use 65 as a representative value for all countries
# This matches the reference age used for multiplier calculation

# Call get_utility_scenario() to obtain utilities for selected scenario
# ----------------------------------------------------------------------
utility_result <- get_utility_scenario(
  country          = country_name,        # Set by active country block ("Ireland", "Germany", or "Spain")
  age              = patient_age,          # Representative patient age (65)
  utility_scenario = utility_scenario,     # Scenario flag defined above
  df_popnorm       = df_popnorm           # Population norm data (loaded earlier)
)
# Returns a list with elements:
#   u_PFS, u_PD (the utilities to use)
#   method, source (metadata describing the approach)
#   U_ref, U_pop, mean_PFS_mcrc, mean_PD_mcrc, mult_PFS_mcrc, mult_PD_mcrc (for population_norm_adjusted scenario)

# Override u_F and u_P with scenario-specific values
# ---------------------------------------------------
u_F <- utility_result$u_PFS
# If ramsey_basecase: u_F = 0.85 (unchanged)
# If population_norm_adjusted: u_F = U_pop_country × mult_PFS_mcrc (country/age-specific)

u_P <- utility_result$u_PD
# If ramsey_basecase: u_P = 0.65 (unchanged)
# If population_norm_adjusted: u_P = U_pop_country × mult_PD_mcrc (country/age-specific)

# Print diagnostic information for transparency
# ----------------------------------------------
cat("\n")
cat("====================================================================\n")
cat("UTILITY SCENARIO APPLIED\n")
cat("====================================================================\n")
cat("Scenario:      ", utility_scenario, "\n", sep = "")
# Prints which scenario is active

cat("Country:       ", country_name, "\n", sep = "")
# Prints which country is being analyzed

cat("Patient age:   ", patient_age, "\n", sep = "")
# Prints the age used for population norm lookup

cat("u_PFS (PFS):   ", round(u_F, 4), "\n", sep = "")
# Prints the PFS utility that will be used in the model (4 decimal places)

cat("u_PD (PD):     ", round(u_P, 4), "\n", sep = "")
# Prints the PD utility that will be used in the model (4 decimal places)

cat("Method:        ", utility_result$method, "\n", sep = "")
# Prints description of the method used (e.g., "Ramsey base case (no adjustment)")

cat("Source:        ", utility_result$source, "\n", sep = "")
# Prints citation/source for the utilities

# If population_norm_adjusted scenario, print additional details
# ---------------------------------------------------------------
if (!is.null(utility_result$U_pop) && !is.na(utility_result$U_pop)) {
  # Check if U_pop exists and is not NA (only present in population_norm_adjusted scenario)

  cat("\nPOPULATION NORM ADJUSTMENT DETAILS:\n")
  cat("-----------------------------------\n")

  cat("Ref country:   ", utility_result$ref_country, " (age ", utility_result$ref_age, ")\n", sep = "")
  # Prints reference country used for multiplier calculation (England (UK), age 65)

  cat("U_ref (UK 65): ", round(utility_result$U_ref, 4), "\n", sep = "")
  # Prints UK population norm at age 65 (denominator for multipliers)

  cat("U_pop (target):", round(utility_result$U_pop, 4), "\n", sep = "")
  # Prints target country population norm at specified age

  cat("mean_PFS_mcrc: ", round(utility_result$mean_PFS_mcrc, 4), "\n", sep = "")
  # Prints mean PFS utility from mCRC literature

  cat("mean_PD_mcrc:  ", round(utility_result$mean_PD_mcrc, 4), "\n", sep = "")
  # Prints mean PD utility from mCRC literature

  cat("mult_PFS_mcrc: ", round(utility_result$mult_PFS_mcrc, 4),
      " (mCRC PFS utility is ", round(utility_result$mult_PFS_mcrc * 100, 1), "% of UK norm)\n", sep = "")
  # Prints PFS multiplier and interpretation

  cat("mult_PD_mcrc:  ", round(utility_result$mult_PD_mcrc, 4),
      " (mCRC PD utility is ", round(utility_result$mult_PD_mcrc * 100, 1), "% of UK norm)\n", sep = "")
  # Prints PD multiplier and interpretation

  cat("\nCALCULATION:\n")
  cat("u_PFS = U_pop × mult_PFS_mcrc = ", round(utility_result$U_pop, 4),
      " × ", round(utility_result$mult_PFS_mcrc, 4),
      " = ", round(u_F, 4), "\n", sep = "")
  # Shows the calculation step-by-step for transparency

  cat("u_PD  = U_pop × mult_PD_mcrc  = ", round(utility_result$U_pop, 4),
      " × ", round(utility_result$mult_PD_mcrc, 4),
      " = ", round(u_P, 4), "\n", sep = "")
  # Shows the calculation step-by-step for transparency
}
...
# NOTE: This diagnostic output can be silenced by commenting out the cat() statements
#       if needed for production runs, but is useful for transparency during development
#       and for demonstrating to reviewers exactly how utilities were calculated.
...
# Test whether the function works (and generates the same results)
# Now passing curve_choice to control which parametric distribution is used
# This allows the function to switch between Weibull (base case) and gamma (scenario)
oncologySemiMarkov(l_params_all = l_params_all, n_wtp = n_wtp, curve_choice = curve_choice)
